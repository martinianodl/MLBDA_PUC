# Databricks notebook source exported at Fri, 11 Nov 2016 16:52:24 UTC
# MAGIC %md
...
uniqueHostPerDay = daily_hosts_df.select(daily_hosts_df['day'], daily_hosts_df['count'].alias('uniqueHost'))

avg_daily_req_per_host_df = total_req_per_day_df.join(uniqueHostPerDay, total_req_per_day_df.day == uniqueHostPerDay.day, 'outer').select(total_req_per_day_df['day'], (total_req_per_day_df['count'] / uniqueHostPerDay['uniqueHost']).alias('avg_reqs_per_host_per_day'))
...
not_found_paths_df = not_found_df.select(not_found_df['path'], not_found_df['status'])
...
errors_by_date_sorted_df = not_found_df.select(logs_df['host'], dayofmonth('time').alias('day')).groupBy('day').count()
