# Databricks notebook source exported at Fri, 11 Nov 2016 16:51:24 UTC
# MAGIC %md
...
# TODO: Replace <FILL IN> with appropriate code
parsed_data_df = parsed_points_df.select((parsed_points_df['label'] - min_year).alias('label'), parsed_points_df['features'])

# parsed_data_df_boiko = parsed_points_df.select("features", "label").withColumn("label", lit(parsed_points_df.label - min_year))

parsed_data_df = parsed_data_df.select(parsed_data_df['features'], parsed_data_df['label'])
...
preds_and_labels_train = parsed_train_data_df.select(parsed_train_data_df['label'])
preds_and_labels_train_df = preds_and_labels_train.withColumn("prediction", lit(average_train_year))
rmse_train_base = calc_RMSE(preds_and_labels_train_df)

# preds_and_labels_train_df.sort('label').show(5)

preds_and_labels_val = parsed_val_data_df.select(parsed_val_data_df['label'])
preds_and_labels_val_df = preds_and_labels_val.withColumn("prediction", lit(average_train_year))
rmse_val_base = calc_RMSE(preds_and_labels_val_df)

preds_and_labels_test = parsed_test_data_df.select(parsed_test_data_df['label'])
preds_and_labels_test_df = preds_and_labels_test.withColumn("prediction", lit(average_train_year))
...
def linreg_gradient_descent(train_data, num_iters):
    ...
    for i in range(num_iters):
        ...
        preds_and_labels_train = train_data.map(lambda o: get_labeled_prediction(w, o))
        ...
        gradient = train_data.map(lambda o: gradient_summand(w, o)).sum()
#         gradient.
