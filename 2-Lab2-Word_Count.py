# Databricks notebook source exported at Fri, 11 Nov 2016 16:52:31 UTC
# MAGIC %md
...
# TODO: Replace <FILL IN> with appropriate code
averageCount = (wordCountsDF.groupBy().mean().collect())
averageCount = float(averageCount[0]['avg(count)'])
print averageCount
...
def removePunctuation(column):
    ...
    column = lower(column)
    column = regexp_replace(column, '[^a-z0-9\s]', '')
    column = trim(column)
...
shakeWordsDF = (shakeWordsDF.select(explode(shakeWordsDF.word).alias('word')).where("word != ''"))
